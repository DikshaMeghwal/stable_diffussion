...
from models.UNet_VAE import UNet as VAE
...
def train_for_one_epoch(epoch_idx, model, mnist_loader, optimizer, crtierion, config):
    ...
    for im, label in tqdm(mnist_loader):
        ...
        print(f"mean:{mean} log_variance:{log_variance}")
        if config['train_params']['save_training_image']:
            org_img = make_grid(im)
            recon_img = make_grid(generated_im)
            save_image(org_img, f"{config['train_params']['task_name']}/{epoch_idx}_org.png")
            save_image(recon_img, f"{config['train_params']['task_name']}/{epoch_idx}_recon.png")
